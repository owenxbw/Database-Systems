...
--Show the count of all the actors who acted in multiple movies.
SELECT COUNT(*) FROM (
	SELECT aid FROM MovieActor GROUP BY aid HAVING COUNT(aid) > 1
) AS A;

--Show the title of movies that sell more than 1,000,000 tickets.
...
-- Retrieve number of movies in the "Adult" genre.
